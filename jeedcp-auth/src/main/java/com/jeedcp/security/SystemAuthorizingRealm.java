...
public class SystemAuthorizingRealm extends AuthorizingRealm {
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authcToken) {
		...
		// 校验登录验证码
		if (!mobile &&isValidateCodeLogin(token.getUsername(), false, false)){
		...
		if (user != null) {
			...
			return new SimpleAuthenticationInfo(new Principal(user, token.isMobileLogin(), (String)UserUtils.getSession().getId()),
	...
	protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
		...
		if (user != null) {
			...
			LogUtils.saveLog(httpServletRequest, "系统登录");
			return info;
		} else {
			return null;
		}
