...
import com.jeedcp.modules.cms.dao.ArticleDao;
...
public class ArticleService extends CrudService<ArticleDao, Article> {
	...
	public Page<Article> findPage(Page<Article> page, Article article, boolean isDataScopeFilter) {
		// 更新过期的权重，间隔为“6”个小时
		Date updateExpiredWeightDate =  (Date)ehCacheServiceBean.get("updateExpiredWeightDateByArticle");
		if (updateExpiredWeightDate == null || (updateExpiredWeightDate != null 
				&& updateExpiredWeightDate.getTime() < new Date().getTime())){
			dao.updateExpiredWeight(article);
			ehCacheServiceBean.put("updateExpiredWeightDateByArticle", DateUtils.addHours(new Date(), 6));
		}
...
//		return dao.find(page, dc);
	...
	public void save(Article article) {
		...
		article.setUpdateBy(CurrentUserUtils.getUser().getId());
		...
		if (StringUtils.isBlank(article.getId())){
			CurrentUserUtils.preInsert(article);
			articleData = article.getArticleData();
			articleData.setId(article.getId());
			dao.insert(article);
			articleDataDao.insert(articleData);
		}else{
            CurrentUserUtils.preUpdate(article);
			articleData = article.getArticleData();
			articleData.setId(article.getId());
			dao.update(article);
