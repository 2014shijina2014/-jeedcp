 ...
 */
package com.jeedcp.modules.act.service;
...
import com.jeedcp.modules.act.dao.ActDao;
import com.jeedcp.modules.act.entity.Act;
import com.jeedcp.modules.act.service.cmd.CreateAndTakeTransitionCmd;
import com.jeedcp.modules.act.service.cmd.JumpTaskCmd;
import com.jeedcp.modules.act.service.creator.ChainedActivitiesCreator;
import com.jeedcp.modules.act.service.creator.MultiInstanceActivityCreator;
import com.jeedcp.modules.act.service.creator.RuntimeActivityDefinitionEntityIntepreter;
import com.jeedcp.modules.act.service.creator.SimpleRuntimeActivityDefinitionEntity;
...
public class ActTaskService extends BaseService {
	...
	public List<Act> todoList(Act act){
		String userId = CurrentUserUtils.getUser().getLoginName();//ObjectUtils.toString(UserUtils.getUser().getId());
...
//			e.setProcExecUrl(ActUtils.getProcExeUrl(task.getProcessDefinitionId()));
	 ...
	 * @return
	 */
	public Pagination<Act> historicList(Pagination<Act> page, Act act){
		String userId = CurrentUserUtils.getUser().getLoginName();//ObjectUtils.toString(UserUtils.getUser().getId());
...
//					List<HistoricIdentityLink> il = historyService.getHistoricIdentityLinksForProcessInstance(procInsId);
					if (il.size() > 0){
						if (StringUtils.isNotBlank(il.get(0).getStartUserId())){
							User user = CurrentUserUtils.getByLoginName(il.get(0).getStartUserId());
							if (user != null){
				...
				if (StringUtils.isNotEmpty(histIns.getAssignee())){
					User user = CurrentUserUtils.getByLoginName(histIns.getAssignee());
					if (user != null){
						e.setAssignee(histIns.getAssignee());
						e.setAssigneeName(user.getName());
					}
	...
	public void completeFirstTask(String procInsId, String comment, String title, Map<String, Object> vars){
		String userId = CurrentUserUtils.getUser().getLoginName();
