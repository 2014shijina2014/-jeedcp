...
<body>
	...
	<form:form id="searchForm" modelAttribute="user" action="${ctx}/sys/user/list" method="post" class="breadcrumb form-search ">
		...
		<ul class="ul-form">
			...
			<li class="btns"><input id="btnSubmit" class="btn btn-primary" type="submit" value="查询" onclick="javascript:page(1);"/>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<tbody>
		<c:forEach items="${page.list}" var="userrow">
			<tr>
				<td>${userrow.company.name}</td>
				<td>${userrow.office.name}</td>
				<td><a href="${ctx}/sys/user/form?id=${userrow.id}">${userrow.loginName}</a></td>
				<td>${userrow.name}</td>
				<td>${userrow.phone}</td>
				<td>${userrow.mobile}</td><%--
				<td>${userrow.roleNames}</td> --%>
				<shiro:hasPermission name="sys:user:edit"><td>
    				<a href="${ctx}/sys/user/form?id=${userrow.id}">修改</a>
					<a href="${ctx}/sys/user/delete?id=${userrow.id}&officeId=${user.office.id}&<sys:pageUrlParam/>" onclick="return confirmx('确认要删除该用户吗？', this.href)">删除</a>
